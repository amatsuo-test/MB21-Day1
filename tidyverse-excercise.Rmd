...
df_vac %>%
  group_by(Date) %>%
  summarize(Series_Complete_Pop_Pct = sum(Series_Complete_Pop_Pct, na.rm = T)) %>%
  filter(Series_Complete_Pop_Pct > 0 ) %>%
  ggplot(aes(x = Date, y = Series_Complete_Pop_Pct)) + geom_line() +
...
df_vac %>%
  ...
  summarize(Series_Complete_Pop_Pct = sum(Series_Complete_Pop_Pct, na.rm = T)) %>%
  filter(Series_Complete_Pop_Pct > 0 ) %>%
  ggplot(aes(x = Date, y = Series_Complete_Pop_Pct, color = Recip_State)) + geom_line() +
  scale_x_date(date_labels = "%y-%b")
